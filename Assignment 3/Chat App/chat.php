...
//connect to database
$db = mysqli_connect('mariadb', 'cs431s28', 'cee9FaTh', 'cs431s28');
if (mysqli_connect_errno()) {
   echo '<p>Error: Could not connect to database.<br/>
   Please try again later.</p>';
   exit;
}
//helper function to replace get_results() if without mysqlnd 
...
try { 
    ...
    switch($action) {
        case 'poll':
           ...
           $stmt->execute();
           $stmt->bind_result($id, $message, $session_id, $date_created, $username, $color); // Add the username and color.
        ...
        'messages' => $newChats
           ...
           exit;
        case 'send':
            // In addition to the parameters from the chatlog table, also include the username and color.
